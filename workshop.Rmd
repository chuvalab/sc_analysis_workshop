...
## Load the dataet
By just running the just loaded object, we get some basic information about it: it has 57490 features (genes) across 838 samples (cells)
```{r}
...
VlnPlot(object, features = c("LEFTY2", "RIPPLY2", "DLL3"))
```
...
```{r, fig.height=6, fig.width=6}
heatmap.2(meanDF_select, col="bluered", trace = "none", dendrogram='column', Rowv=FALSE,  Colv=TRUE)
```
...
```{r, fig.height=6, fig.width=6}
heatmap.2(meanDF_select, col="bluered", 
          trace='none', 
          scale='row')
```
